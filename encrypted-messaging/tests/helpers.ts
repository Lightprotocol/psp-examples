...
import {ConfirmOptions, CONSTANT_SECRET_AUTHKEY, SendVersionedTransactionsResult, User} from "@lightprotocol/zk.js";
...
export class MessageClient {
    ...
    async encryptAndStoreForRecipient(message: string, recipient: Uint8Array): Promise<SendVersionedTransactionsResult> {
    ...
    async store(message: Buffer, anonymousSender: boolean = false): Promise<SendVersionedTransactionsResult> {
        let res = await this.user.storeData(message, ConfirmOptions.spendable, !anonymousSender);
